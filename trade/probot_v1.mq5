...
void OnTick()
{
   ...
   if(positions == 1 && pendingExists)
   {
      ...
      double tp1ProfitEst = InpTPPoints * InpLot1
               ...
               double point, int digits)
{
   ...
   if(dir == ORDER_TYPE_BUY)
   {
      ...
      Print("BUY cycle | E:", entry, " TP:", tp1, " SL:", sl1,
            " Strike@:", strikePrice, " Lot2:", lot2,
            InpE2EmergSLPts > 0 ? StringFormat(" E2-SL:%.5f", e2sl) : " E2-SL:OFF");

      if(!trade.Buy(InpLot1, _Symbol, entry, sl1, tp1, "E1-BUY"))
      {
   ...
   else
   {
      ...
      if(!trade.Sell(InpLot1, _Symbol, entry, sl1, tp1, "E1-SELL"))
      {
