...
bool ComputeAutoBestHedgePlan(const double main_entry_price,
                              ...
                              string &reason)
  {
   ...
   if(point_money <= 0.0 || InpHedgeSlPoints <= 0.0)
     {
      reason = "Invalid symbol or hedge SL settings";
      return false;
     }

   const TradeSide hedge_side = (InpMainSide == SideBuy) ? SideSell : SideBuy;
   const double main_tp_money = TradePnlMoney(InpMainSide, main_entry_price, main_tp_price, main_lots);
   const double main_sl_money = TradePnlMoney(InpMainSide, main_entry_price, main_trigger_price, main_lots);
   ...
   const double hedge_sl_per_lot = TradePnlMoney(hedge_side, main_trigger_price, hedge_sl_price, 1.0);
   if(hedge_sl_per_lot >= 0.0)
     {
      reason = "Invalid hedge SL direction";
      return false;
     }

   const double min_path_profit = MathMax(0.0, InpMinPathNetUSD);
     const double be_profit_target = MathMax(min_path_profit,
           MathMax(MathAbs(main_tp_money) * (InpBeTargetProfitPct / 100.0),
             (InpUseBasketSafety ? EffectiveBasketTakeProfitUsd() : 0.0)));
   const double full_profit_target = MathMax(be_profit_target,
                                   MathMax(min_path_profit,
                                           MathAbs(main_tp_money) * (InpRecoveryProfitPct / 100.0)));

   const double max_lots_raw = (main_tp_money - min_path_profit) / MathAbs(hedge_sl_per_lot);
   hedge_lots = NormalizeLotsDown(max_lots_raw);
   if(hedge_lots <= 0.0)
     {
      reason = "Computed hedge lots invalid";
      return false;
   ...
   double per_point_net = hedge_lots * point_money;
   if(InpKeepMainOpenOnHedge)
     {
      per_point_net = (hedge_lots - main_lots) * point_money;
      if(per_point_net <= 0.0)
        {
         reason = StringFormat("No feasible lots: hedge %.2f must exceed main %.2f", hedge_lots, main_lots);
         return false;
        }
     }

   be_points_needed = (MathAbs(main_sl_money) + be_profit_target) / per_point_net;
   tp_points_needed = (MathAbs(main_sl_money) + full_profit_target) / per_point_net;
   if(be_points_needed <= 0.0 || tp_points_needed <= 0.0)
     {
      reason = "Computed hedge distances invalid";
      return false;
     }

   hedge_be_price = PriceByPoints(hedge_side, main_trigger_price, be_points_needed, true);
   hedge_tp_price = PriceByPoints(hedge_side, main_trigger_price, tp_points_needed, true);

   const double net_main_tp_hedge_sl = main_tp_money + (hedge_sl_per_lot * hedge_lots);
   double net_main_sl_hedge_tp = 0.0;
   if(InpKeepMainOpenOnHedge)
      net_main_sl_hedge_tp = TradePnlMoney(InpMainSide, main_entry_price, hedge_tp_price, main_lots)
                             + TradePnlMoney(hedge_side, main_trigger_price, hedge_tp_price, hedge_lots);
   else
      net_main_sl_hedge_tp = main_sl_money + TradePnlMoney(hedge_side, main_trigger_price, hedge_tp_price, hedge_lots);

   if(net_main_tp_hedge_sl < min_path_profit)
     {
      reason = StringFormat("Main TP + Hedge SL below target: %.2f < %.2f", net_main_tp_hedge_sl, min_path_profit);
      return false;
     }
   if(net_main_sl_hedge_tp < full_profit_target)
     {
      reason = StringFormat("Main SL + Hedge TP below target: %.2f < %.2f", net_main_sl_hedge_tp, full_profit_target);
      return false;
     }
   ...
   if(InpAutoBestHedge)
     {
      ...
      if(!ComputeAutoBestHedgePlan(entry_price,
                                   main_sl_price,
                                   main_tp_price,
                                   main_lots,
                                   hedge_lots,
...
bool PlaceHedgePendingFromMain()
  ...
  if(InpAutoBestHedge)
    ...
    {
     const double point_money = MoneyPerPointPerLot();
     const double main_sl_money = TradePnlMoney(InpMainSide, g_main_entry_actual, main_trigger, g_main_lots_actual);
     const double main_tp_money = TradePnlMoney(InpMainSide, g_main_entry_actual, g_main_tp_actual, g_main_lots_actual);
     hedge_tp_raw = PriceByPoints(hedge_side, main_trigger, InpBestHedgeTpPoints, true);
     const double pnl_per_lot_tp = TradePnlMoney(hedge_side, main_trigger, hedge_tp_raw, 1.0);

     if(point_money <= 0.0 || pnl_per_lot_tp <= 0.0)
       return false;

     const double recovery_extra = MathAbs(main_tp_money) * (InpRecoveryProfitPct / 100.0);
     hedge_sl_raw = PriceByPoints(hedge_side, main_trigger, InpHedgeSlPoints, false);
     const double pnl_per_lot_sl = TradePnlMoney(hedge_side, main_trigger, hedge_sl_raw, 1.0);
     ...
     if(InpKeepMainOpenOnHedge)
       {
        const double per_point_net = (hedge_lots - g_main_lots_actual) * point_money;
        if(per_point_net <= 0.0)
          return false;
     ...
     hedge_be_price = PriceByPoints(hedge_side, main_trigger, be_points_needed, true);
    }
  ...
  double hedge_sl = 0.0;
  double hedge_tp = 0.0;
  // With keep-main-open, do NOT set a hard TP: hedge would close at profit while main is still
  // losing, leaving the basket unhedged. ManageOpenHedge / BasketSafety handle the exit instead.
  const double hedge_tp_for_order = InpKeepMainOpenOnHedge ? 0.0 : hedge_tp_raw;
...
string BuildScenarioReport()
  ...
  report += "Stake: " + DoubleToString(main_lots, 2) + " lots"
   ...
   if(!InpAutoBestHedge)
   ...
   else
     {
      ...
      if(InpKeepMainOpenOnHedge)
        report += "Basket @ BE target: " + DoubleToString(best_net_if_be, 2)
               + " | Basket @ hedge TP: " + DoubleToString(best_net_if_tp, 2);
      else
  ...
  SetPanelLine(row++, "  Risk: $" + DoubleToString(main_risk_usd, 2) + "  (" + DoubleToString(main_risk_pct, 1) + "% of stake)"
   ...
   if(!InpAutoBestHedge)
    ...
    SetPanelLine(row++, "  BE distance  : " + DoubleToString(be_points_needed, 0) + " pts", clrSilver);
    SetPanelLine(row++, "  TP distance  : " + DoubleToString(tp_points_needed, 0) + " pts  [auto-fit]", clrLime);
      if(InpKeepMainOpenOnHedge)
        {
         SetPanelLine(row++, "  Basket @ BE tgt  : " + (best_net_if_be >= 0 ? "+" : "") + DoubleToString(best_net_if_be, 2), PnlColor(best_net_if_be));
         SetPanelLine(row++, "  Basket @ hedge TP: " + (best_net_if_tp >= 0 ? "+" : "") + DoubleToString(best_net_if_tp, 2), PnlColor(best_net_if_tp));
        }
      else
        {
         SetPanelLine(row++, "  Net @ BE target  : " + (best_net_if_be >= 0 ? "+" : "") + DoubleToString(best_net_if_be, 2), PnlColor(best_net_if_be));
         SetPanelLine(row++, "  Net @ full target: " + (best_net_if_tp >= 0 ? "+" : "") + DoubleToString(best_net_if_tp, 2), PnlColor(best_net_if_tp));
        }
     }
   SetPanelLine(row++, sep, clrDimGray);
  ...
  SetPanelLine(row++, "  B) Main TP + Hedge SL     => $" + DoubleToString(s2, 2), PnlColor(s2 - sim));
   ...
   const double s3 = sim + net_main_sl_with_hedge_tp;
